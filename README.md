# 关于ChainHis
ChainHis出发点是创建一本去中心化的历史书，是一个偏文化的区块链项目。如果本项目长期能长期运行下去，那么所有的NFT串在一起就会形成一部记录在区块链上的编年史。  
为了在项目内形成内循环激励机制，并确保项目长期生存，所以设置了分红和拍卖的机制。但是出发点不是为了割韭菜或者圈钱。**项目方是纯开发人员，不会拉盘，不会托市，也不计划发币，也不计划套娃**，希望大家理性参与。  
项目是业余时间开发，早期开发和参与的人都很少，希望以后能够得到更多朋友的支持和参与。  
...
history和card还有部分开发工作（主要是秘书投票功能），暂未开源，预计在三个月内实现全部功能后会全部提交源代码。  
[项目网站入口](https://www.chainhis.net)已经初步可用，还需要一段时间才能提供完整功能（合并图片和新闻文字呈现）。  
...
* NFT持有人的奖励。早期的NFT持有人，可以平分后来的NFT拍卖金额的10%（包括自己这个NFT的奖励）。NFT持有人可以通过命令如"history settleCard -l 2023-06-23 -r 2023-06-24"结算这部分奖励，可以通过比如"hisotry dumpReward -l 2023-06-22 -r 2023-06-23" 命令查看是否已经结算。**注意:后续合约会限定只能参与领取后续365个NFT的分红。因为随着NFT个数增多，每个NFT分得的奖励会随着时间越来越少，到很多年后可能就毫无激励意义。所以在后续版本合约中会限制领取的范围。**
...
按照目前的设想，在代码全部稳定后，开发团队会放弃owner权限，其5%的奖励作为社区的公积金留存备用，也可能直接划转给投票人。  

**2023-08-01合约升级后分红调整如下**
* 分红比例有所调整，最新比例为开发分红0%，秘书分红6%，持卡人12%，投票人82%
* 持卡人只能领取后续365个和自己合计366个奖励
